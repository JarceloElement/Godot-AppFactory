...
export var field_instance_name = "line_edit_panel"
...
var table_name = "USERS_FORM"
var Path_DB_form = "from_DB_control"
...
#			print("total_report FORM:",query["result"].size())
	...
	if total_report == 0:
		# --- NO RESULT SEARCH ---
		var param_msg = [get_node("/root/Messages").no_result_search_icon,get_node("/root/Messages").no_result_search]
...
func _process(delta):
	...
	if to_list > 0 and to_list < 5: # espera un tiempo antes de agregar los items
		to_list += 1
	
	# si el menu esta cerrado
	if get_node("/root/FuncApp").menu_hide == 1 and to_list >= 5:
...
func click_awesome(id_message,param):
	...
	if id_message == "add_DB_control_from_category" or id_message == "add_HTTP_control_from_category":
		var form_item = get_tree().get_nodes_in_group("form_item")
		for i in form_item:
			i.form_item_delet() # va a los campos_form o reportes para eliminarlos
			pass
